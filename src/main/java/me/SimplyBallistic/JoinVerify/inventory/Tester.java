...
public class Tester {
	...
	public Tester(Player p, Plugin plugin, Runnable onPass,Runnable onWrong,List<String> sBlocks,String pickName,String itemName,String guiName) {
		...
		testMenu=new IconMenu(guiName.replaceAll("%item%",blocks[answer].getType().toString().replaceAll("_", " ") ), 27, event->{
	...
	private void initMenu(IconMenu menu,ItemStack[]blocks){
		...
		for(int i=0;i<27;i++){
			if(i<10||i>16){ 
				...
				menu.setOption(i, blocks[answers], itemName.replaceAll("%item%", blocks[answers].getType().toString()), 
						answers==answer ? pickName :"");//this is temp
				answers++;
